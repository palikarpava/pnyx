	...
	class TP_Request {
		...
		*/
		...
		public function __construct( $id = false, $choices = array() ) {
			...
			$this->follow_up    = isset( $_REQUEST['totalpoll']['follow_up'] ) ? $_REQUEST['totalpoll']['follow_up'] : array();
			...
			if ( $this->id ):
				if ( get_post_meta( $this->id, 'choices', true ) !== false ):
					...
					add_action( 'totalpoll/actions/request/follow_up', array( $this, 'follow_up' ) );
				endif;
			endif;
		...
		public function vote() {
			...
			if ( $this->poll !== false ): // Check poll
				...
				include(TP_PATH . "includes/customfile/country.php");
		...
		public function follow_up() {
			...
			$follow_up_obj = $this->poll->follow_up();

			$follow_up_question = $follow_up_obj->get_follow_up_question();

			$validate_obj = array();
			if (!$follow_up_obj->is_email_provided()):
				$validate_obj["email"] = $this->follow_up["email"];
				$validate_obj["agreement"] = $this->follow_up["agreement"];
			endif;
			if (!$follow_up_obj->is_follow_up_question_addressed()):
				$validate_obj["answer"] = $this->follow_up["answer"];
			endif;

			$follow_up_obj->validate($validate_obj);

			if (empty( $follow_up_obj->errors() ) === true):
				if (!$follow_up_obj->is_email_provided()):
					$follow_up_obj->save_email_address($this->follow_up["email"]);
				endif;
				if (!$follow_up_obj->is_follow_up_question_addressed()):
					$SxGeo = new SxGeo(TP_PATH . "includes/geo/SxGeoCity.dat");
					$city = $SxGeo->getCityFull($this->ip);

					$follow_up_item = array(
						"question" => $follow_up_question,
						"time"		=> current_time( 'timestamp' ),
						"answer"	=> $this->follow_up["answer"],
						"country"	=> isset($city["country"]) ? $city["country"]["name_en"] : "unknown",
						"region"	=> isset($city["region"]) ? $city["region"]["name_en"] : "unknown",
						"city"		=> isset($city["city"]) ? $city["city"]["name_en"] : "unknown",
					);
					$follow_up_obj->save_follow_up_response($follow_up_item);
				endif;

				$follow_up_obj->remove_redirect_to_follow_up();
