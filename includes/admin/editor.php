...
class TP_Admin_Editor {
	...
	public function __construct() {
		...
		add_filter( 'enter_title_here', array( $this, 'change_poll_title_text' ) );
		add_filter( 'default_content', array( $this, 'get_default_question_text' ) );
		...
		$this->defaults = array(
			'limitations' =>
				array(
					'cookies'    => array( 'timeout' => '525600' ),
					'ip'         => array( 'timeout' => '525600', 'filter' => '' ),
			...
			'fields'      => array(),
			'screens'     => array(),
			'charts'	  => array('order' => '2'),
			'country'	  => array('map' => 'disabled'),
			'logs'        => array(
				'enabled' => false,
			),
		);
	...
	public function assets() {
		...
		wp_enqueue_script( 'pn-font-awesome', TP_URL . 'assets/js/fontawesome-all.min.js', array(), ( WP_DEBUG ? time() : TP_VERSION ) );
	 ...
	 * @return bool|void
	 */
	public function save( $poll_id , $post, $update) {
		...
		if ( empty( $totalpoll['download'] ) === false ):
			...
			elseif ( isset( $totalpoll['download']['follow-up-responses'] ) === true ):
				$format = in_array( $totalpoll['download']['follow-up-responses'], $formats ) ? $totalpoll['download']['follow-up-responses'] : $formats[0];
				...
				$responses = $meta_pageable->paginate( 'follow-up-responses', $poll_id, 1, - 1 );

				foreach ( $responses as $item ):
					$content[] = array(
						date( 'Y-m-d H:i e', $item['time'] ),
						$item['question'],
						$item['answer'],
						$item['country'],
						$item['city']
		...
		if ( empty( $totalpoll['reset'] ) === false ):
			...
			if ( isset( $totalpoll['reset']['follow-up-responses'] ) === true ):
				$meta_pageable->reset( 'follow-up-responses', $poll_id );
			endif;
		...
		if ( isset( $totalpoll["videoquestion"]["url"] )){
			update_post_meta( $poll_id, "videoquestion",  $totalpoll["videoquestion"]["url"] );
		}else{
			delete_post_meta( $post_id, "videoquestion");
		}

		if ( isset( $totalpoll["videoquestion"]["thumbnail"] )){
			update_post_meta( $poll_id, "videoquestionimg",  $totalpoll["videoquestion"]["thumbnail"] );
		}else{
			delete_post_meta( $post_id, "videoquestionimg");
		}

        if (isset($settings['results']['order']['request_email'])) {
            update_post_meta( $poll_id, 'request_email',  'on' );
        } else {
		    delete_post_meta( $poll_id, 'request_email', 'off');
        }

		if (isset($settings['design']['template']['display_image'])) {
            update_post_meta( $poll_id, 'display_image',  'on' );
        } else {
            update_post_meta( $poll_id, 'display_image', 'off');
        }

        if (isset($settings['limitations']['thiscategory']['enabled'])) {
            update_post_meta( $poll_id, 'thiscategory',  'on' );
        } else {
            delete_post_meta( $poll_id, 'thiscategory');
        }
		...
		// clear featured flag
		if ( ! empty( $settings['limitations']['allcategory'] ) ){
			$rd_args = array(
				'post_type' => 'poll',
				'meta_key' => 'allcategory',
				'meta_value' => 'on'
			);
			$q = new WP_Query();
			$_posts = $q->query( $rd_args );
			foreach ( (array) $_posts as $_post ) {
				$_limitations = get_post_meta($_post->ID, 'settings_limitations');
				unset($_limitations[0]['allcategory']);

				update_post_meta($_post->ID, 'settings_limitations', $_limitations[0]);
				delete_post_meta($_post->ID, 'allcategory');
			}
			update_post_meta($poll_id, 'allcategory', 'on');
		}else{
			delete_post_meta($poll_id, 'allcategory');
		}

        if (!empty($settings['limitations']['thiscategory'])) {
            $terms = get_the_terms($poll_id, 'poll_category');

            $thiscat = null;
            if (class_exists('WPSEO_Primary_Term')) {
                // Show the post's 'Primary' category, if this Yoast feature is available, & one is set
                $wpseo_primary_term = new WPSEO_Primary_Term('poll_category', $poll_id);
                $wpseo_primary_term = $wpseo_primary_term->get_primary_term();
                $term = get_term($wpseo_primary_term);
                if (!is_wp_error($term)) {
                    $thiscat = $term->term_id;
                    $category_link = get_category_link($term->term_id);
                }
                //	var_dump($category_display);
            } else {
                if ($terms) {
            ...
            if($thiscat){
                ...
                foreach($posts as $_post){
                    if($_post->ID != $poll_id){
                        $_limitations = get_post_meta($_post->ID, 'settings_limitations');
                        unset($_limitations[0]['thiscategory']);
                        update_post_meta($_post->ID, 'settings_limitations', $_limitations[0]);
                    }
                }
            }
		...
		if ( $template == false ):
			...
			elseif ( isset($settings['design']['template']['last_used']) && $settings['design']['template']['last_used'] !== $settings['design']['template']['name'] ):
				$settings['design']['preset'] = array();
			endif;
		endif;
	...
	// Set default category if no categories selected
        if(!isset($_POST['tax_input']['poll_category']) || (count($_POST['tax_input']['poll_category']) == 1 && $_POST['tax_input']['poll_category'][0] == 0)){
            wp_set_post_terms($poll_id, get_option('totalpoll_default_category_id'), 'poll_category');
}
	...
	function change_poll_title_text( $title ){
	     return 'Type your poll title here...';
	}

	function get_default_question_text($content){
