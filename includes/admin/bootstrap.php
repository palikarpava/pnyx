...
if ( ! class_exists( 'TP_Admin_Bootstrap' ) ) :
	...
	class TP_Admin_Bootstrap {
		...
		public function __construct() {
			...
			add_action( 'admin_enqueue_scripts', array( $this, 'admin_categories_assets' ) );

			do_action( 'totalpoll/actions/admin/init' );

			add_filter('manage_edit-poll_category_columns' , array($this, 'category_column_title'));
            add_filter('manage_poll_category_custom_column' , array($this, 'category_column_content'), 10, 3);
		}

		public function category_column_title( $columns )
        {
            $columns['is_default'] = __('Default');
            return $columns;
        }

        function category_column_content( $content, $column_name, $term_id )
        {
            if ( 'is_default' == $column_name ) {
                $checked = $term_id == get_option('totalpoll_default_category_id');
                $content = '<div style="padding-left: 16px;"><input '.($checked ? 'checked' : '').' type="checkbox" class="is_default_checkbox"></div>';
            }
            return $content;
        }
		...
		public function menus() {
			...
			// Extensions
			add_submenu_page( $menu_slug, __( 'About', TP_TD ), __( 'About', TP_TD ), 'edit_posts', 'tp-about', array( $this, 'page' ), 10, 3 );
			add_submenu_page( $menu_slug, __( 'Support Center', TP_TD ), __( 'Support / Help', TP_TD ), 'edit_posts', 'tp-support', array( $this, 'page' ), 10, 3 );
		}
		...
		public function admin_categories_assets() {
			wp_enqueue_script( 'tp-admin-categories', TP_URL . 'assets/js/admin-categories.js', array( 'jquery' ), TP_VERSION );
		}
				...
				$pointers['items'] = array(
					'#menu-posts-poll > a'                                                  =>
						array(
							...
							'body'  => __( 'You can access to your polls from this menu.', TP_TD ),
						),
					...
					'#menu-posts-poll a[href="edit.php?post_type=poll&page=tp-support"]'    =>
						array(
							...
							'body'  => __( 'Visit this page if you need any help.', TP_TD ),
						),
				);
			else:
				$pointers['items'] = array(
